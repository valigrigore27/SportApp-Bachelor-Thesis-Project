...
//sa fac o metoda care sa imi ia userul (id + nume)
    ...
    public String myFriends(Model model){
        ...
        for(MyUser friend : allMyFriends){
            if (friend.getPhotoPath() == null || friend.getPhotoPath().isEmpty()) {
                friend.setPhotoPath("user.webp");
            }
        }
    ...
    public String maybeYouKnowThem(Model model){
        ...
        for(MyUser user1 : notMyFriends){
            if (user1.getPhotoPath() == null || user1.getPhotoPath().isEmpty()) {
                user1.setPhotoPath("user.webp");
            }
        }
        model.addAttribute("userId", userId);
        model.addAttribute("username", username);
    ...
    public String myPendingFriends(Model model){
        ...
        for(MyUser pendingFriend : allMyPendingFriends){
            if (pendingFriend.getPhotoPath() == null || pendingFriend.getPhotoPath().isEmpty()) {
                pendingFriend.setPhotoPath("user.webp");
            }
        }
        model.addAttribute("userId", userId);
        model.addAttribute("username", username);
    ...
    public String myFriendRequests(Model model){
        ...
        for(MyUser req : myFriendRequests){
            if (req.getPhotoPath() == null || req.getPhotoPath().isEmpty()) {
                req.setPhotoPath("user.webp");
            }
        }
